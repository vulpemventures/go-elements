...
func TestBroadcastUnblindedIssuanceTxP2PKH(t *testing.T) {
	...
	**/
	...
	utxosAsset, err := unspents(address)
	if err != nil {
		t.Fatal(err)
	}

	// The transaction will have 2 input and 4 outputs.
	txInputHash, _ = hex.DecodeString(utxosAsset[0]["txid"].(string))
	txInputHash = bufferutil.ReverseBytes(txInputHash)
	txInputIndex = uint32(utxosAsset[0]["vout"].(float64))
	txInput = transaction.NewTxInput(txInputHash, txInputIndex)

	txInputAssetHash, _ := hex.DecodeString(utxosAsset[1]["txid"].(string))
	txInputAssetHash = bufferutil.ReverseBytes(txInputHash)
	txInputAssetIndex := uint32(utxosAsset[1]["vout"].(float64))
	txAssetInput := transaction.NewTxInput(txInputAssetHash, txInputAssetIndex)

	asset, _ := hex.DecodeString(
		utxosAsset[1]["asset"].(string),
	)
	asset = append([]byte{0x01}, bufferutil.ReverseBytes(lbtc)...)

	receiverValue, _ := confidential.SatoshiToElementsValue(60000000)
	receiverScript := p2wpkh.Script
	receiverOutput := transaction.NewTxOutput(lbtc, receiverValue[:], receiverScript)

	changeValue, _ = confidential.SatoshiToElementsValue(39999000)
	...
	receiverAssetValue, _ := confidential.SatoshiToElementsValue(1000)
	receiverAssetScript := p2wpkh.Script
	receiverAssetOutput := transaction.NewTxOutput(asset, receiverAssetValue[:], receiverAssetScript)

	// Create a new pset.
	inputs = []*transaction.TxInput{txInput, txAssetInput}
	outputs = []*transaction.TxOutput{receiverOutput, changeOutput, feeOutput, receiverAssetOutput}
	p, err = New(inputs, outputs, 2, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	witValue, _ = confidential.SatoshiToElementsValue(uint64(utxosAsset[0]["value"].(float64)))
	witnessUtxo = transaction.NewTxOutput(lbtc, witValue[:], p2wpkh.WitnessScript)
	err = updater.AddInWitnessUtxo(witnessUtxo, 0)
	if err != nil {
		t.Fatal(err)
	...
	sigWithHashType0 := append(sig.Serialize(), byte(txscript.SigHashAll))

	// Update the pset adding the input signature script and the pubkey.
	_, err = updater.Sign(0, sigWithHashType0, pubkey.SerializeCompressed(), nil, nil)
	if err != nil {
		t.Fatal(err)
	}

	valid, err = updater.Data.ValidateInputSignatures(0)
	if err != nil {
		t.Fatal(err)
	}
	if !valid {
		t.Fatal(errors.New("invalid signature"))
	}
	...
	witValue, _ = confidential.SatoshiToElementsValue(uint64(utxosAsset[1]["value"].(float64)))
	witnessUtxo = transaction.NewTxOutput(asset, witValue[:], p2wpkh.WitnessScript)
	err = updater.AddInWitnessUtxo(witnessUtxo, 1)
	if err != nil {
		t.Fatal(err)
	}

	witHash = updater.Data.UnsignedTx.HashForWitnessV0(1, p2wpkh.Script, witValue[:], txscript.SigHashAll)
	if err != nil {
		t.Fatal(err)
	}
	sig, err = privkey.Sign(witHash[:])
	...
	sigWithHashType1 := append(sig.Serialize(), byte(txscript.SigHashAll))

	// Update the pset adding the input signature script and the pubkey.
	_, err = updater.Sign(1, sigWithHashType1, pubkey.SerializeCompressed(), nil, nil)
	if err != nil {
		t.Fatal(err)
	}
