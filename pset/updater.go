...
func (p *Updater) AddInNonWitnessUtxo(tx *transaction.Transaction, inIndex int) error {
	if inIndex > len(p.Upset.Inputs)-1 {
		return psbt.ErrInvalidPrevOutNonWitnessTransaction
	}

	p.Upset.Inputs[inIndex].NonWitnessUtxo = tx

	if err := p.Upset.SanityCheck(); err != nil {
...
func (p *Updater) AddInWitnessUtxo(txout *transaction.TxOutput, inIndex int) error {
	...
	p.Upset.Inputs[inIndex].WitnessUtxo = txout
...
func (p *Updater) addPartialSignature(inIndex int, sig []byte,
	...
	if pInput.NonWitnessUtxo != nil {
		if len(p.Upset.UnsignedTx.Inputs) < inIndex+1 {
			return psbt.ErrInvalidPrevOutNonWitnessTransaction
		}

		if txHash := pInput.NonWitnessUtxo.TxHash(); !bytes.Equal(txHash[:], p.Upset.UnsignedTx.Inputs[inIndex].Hash) {
	...
	p.Upset.Inputs[inIndex].PartialSigs = append(
		p.Upset.Inputs[inIndex].PartialSigs, &partialSig,
	)
...
func (p *Updater) AddInSighashType(sighashType txscript.SigHashType,
	...
	p.Upset.Inputs[inIndex].SighashType = sighashType
...
func (p *Updater) AddInRedeemScript(redeemScript []byte,
	...
	p.Upset.Inputs[inIndex].RedeemScript = redeemScript
...
func (p *Updater) AddInWitnessScript(witnessScript []byte,
	...
	p.Upset.Inputs[inIndex].WitnessScript = witnessScript
...
func (p *Updater) AddInBip32Derivation(masterKeyFingerprint uint32,
	...
	for _, x := range p.Upset.Inputs[inIndex].Bip32Derivation {
		if bytes.Equal(x.PubKey, bip32Derivation.PubKey) {
			return psbt.ErrDuplicateKey
		}
	}

	p.Upset.Inputs[inIndex].Bip32Derivation = append(
		p.Upset.Inputs[inIndex].Bip32Derivation, &bip32Derivation,
	)
...
func (p *Updater) AddOutBip32Derivation(masterKeyFingerprint uint32,
	...
	// Don't allow duplicate keys
	for _, x := range p.Upset.Outputs[outIndex].Bip32Derivation {
		if bytes.Equal(x.PubKey, bip32Derivation.PubKey) {
			return psbt.ErrDuplicateKey
		}
	}

	p.Upset.Outputs[outIndex].Bip32Derivation = append(
		p.Upset.Outputs[outIndex].Bip32Derivation, &bip32Derivation,
	)
...
func (p *Updater) AddOutRedeemScript(redeemScript []byte,
	...
	p.Upset.Outputs[outIndex].RedeemScript = redeemScript
...
func (p *Updater) AddOutWitnessScript(witnessScript []byte,
	...
	p.Upset.Outputs[outIndex].WitnessScript = witnessScript
...
func (p *Updater) AddInput(txInput *transaction.TxInput) {
	p.Upset.UnsignedTx.AddInput(txInput)
	p.Upset.Inputs = append(p.Upset.Inputs, PInput{})
}

// AddOutput adds output to underlying unsignedTx
func (p *Updater) AddOutput(txOutput *transaction.TxOutput) {
	p.Upset.UnsignedTx.AddOutput(txOutput)
	p.Upset.Outputs = append(p.Upset.Outputs, POutput{})
}
...
func (p *Updater) AddIssuance(arg AddIssuanceArg) error {
	if len(p.Upset.UnsignedTx.Inputs) == 0 {
	...
	for i, input := range p.Upset.UnsignedTx.Inputs {
	...
	p.Upset.UnsignedTx.Inputs[inputIndex].Issuance = &transaction.TxIssuance{
