...
func TestUpdaterAddInput(t *testing.T) {
	...
	assert.Equal(t, 0, len(updater.Upset.UnsignedTx.Inputs))
	assert.Equal(t, 0, len(updater.Upset.Inputs))

	updater.AddInput(&txInput)

	assert.Equal(t, 1, len(updater.Upset.UnsignedTx.Inputs))
	assert.Equal(t, 1, len(updater.Upset.Inputs))
}
...
func TestUpdaterAddOutput(t *testing.T) {
	...
	assert.Equal(t, 0, len(updater.Upset.UnsignedTx.Outputs))
	assert.Equal(t, 0, len(updater.Upset.Outputs))

	updater.AddOutput(&txOutput)

	assert.Equal(t, 1, len(updater.Upset.UnsignedTx.Outputs))
	assert.Equal(t, 1, len(updater.Upset.Outputs))
}
